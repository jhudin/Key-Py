...
from Modulos.VentModificar import *
...
def modif(tab,Modif,top):
    def modifcicarme():
        curItem = TablaDev.item(TablaDev.focus())
        codig = curItem['text']
        codigo=str(codig)
        Modif.withdraw()
        modifvent(tab,codigo,Modif)
    valor=False
    Modificb=Button(Modif,text="Modificar",command=modifcicarme).place(relx=.4,rely=.93)
    if(tab=="Docentes"):
        if(valor):
            TablaDev.destroy()
            valor=False
        ...
        TablaDev.column("Col1", width="120", minwidth="118",
                                stretch="1", anchor="w")
        ...
        conect.close()
        TablaDev.heading("#0", anchor="w")
        TablaDev.column("#0", width="101", minwidth="101",
                                stretch="1", anchor="w")
        TablaDev.heading("Col1", text="Nombre")
        TablaDev.heading("Col1", anchor="w")
