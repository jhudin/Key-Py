...
def reportepdfs(tablaselec):
    class reportePDF(object):
        ...
        def _encabezadoPiePagina(canvas, archivoPDF):
            ...
            # Pie de página
            piePagina = Paragraph("Reporte", estilos["Normal"])
        ...
        def convertirDatos(self):
            ...
            if(tablaselec=="Prestamos"):
                sql = "SELECT COUNT(*) FROM registro_prestamos"
                cursor.execute(sql)
                result=cursor.fetchall()
                conect.close()
                for dato in self.datos:
                    nuevosDatos.append([Paragraph(str(dato[clave]), estiloNormal) for clave in range(6)])
                return nuevosDatos
            elif(tablaselec=="Docentes"):
                sql = "SELECT COUNT(*) FROM docentes"
                cursor.execute(sql)
                result=cursor.fetchall()
                conect.close()
                for dato in self.datos:
                    nuevosDatos.append([Paragraph(str(dato[clave]), estiloNormal) for clave in range(3)])
                return nuevosDatos
            
        def Exportar(self):
            """Exportar los datos a un archivo PDF."""

            alineacionTitulo = ParagraphStyle(name="centrar", alignment=TA_CENTER, fontSize=11,
                                              leading=10, textColor=deepskyblue,
            ...
            tabla.setStyle(TableStyle([
                ("BACKGROUND", (0, 0),(-1, 0), deepskyblue),
                ("ALIGN", (0, 0),(0, -1), "LEFT"),
            ...
            try:
             ...
             # +------------------------------------+
                return messagebox.showinfo("Completado","Reporte generado con exito")
             # +------------------------------------+
            except PermissionError:
             # +--------------------------------------------+  
                return messagebox.showerror("Error inesperado:"," Permiso denegado.")
             # +--------------------------------------------+
    ...
    def generarReporte():
        ...
        cursor = conect.cursor()
        if(tablaselec=="Prestamos"):
            sql = "SELECT `Dia`, `hora`, `Nombre_docent`, `Programa`, `Nombre_llave` , `Novedades` FROM registro_prestamos"
            cursor.execute(sql)
            datos=cursor.fetchall()
            conect.close()

            titulo = "SALAS PRESTADAS"

            cabecera = (
                ("Dia", "Dia"),
                ("hora", "Hora"),
                ("Nombre_docent", "Docente"),
                ("Programa", "Programa"),
                ("Nombre_llave", "LLave"),
                ("Novedades", "Novedades")
                )
            filename1 =  filedialog.asksaveasfilename(initialdir = "/",title = "Select file",defaultextension = 'pdf',filetypes = (("pdf files","*.pdf"),("all files","*.*")))
            nombrePDF = filename1
        elif(tablaselec=="Docentes"):
            sql = "SELECT  `Identificacion_docente`,`Nombre_docente`,`programa` FROM docentes"
            cursor.execute(sql)
            datos=cursor.fetchall()
            ...
            cabecera = (
                ("Identificacion_docente", "ID"),
                ("Nombre_docente", "Docente"),
                ("programa", "Programa")
                )
            filename =  filedialog.asksaveasfilename(initialdir = "/",title = "Select file",defaultextension = 'pdf',filetypes = (("pdf files","*.pdf"),("all files","*.*")))
